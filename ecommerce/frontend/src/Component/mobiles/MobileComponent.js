...
import '../../index.css';
import useGetAllItemsByComapanyName from '../utility/useGetItemByComepanyName.js';
...
export default function MobileComponent(props) {
    ...
    const [Allphone, setAllPhones] = useState([])

    const Apple = useGetAllItemsByComapanyName('mobile', 'Apple');
    ...
    useEffect(() => {
        if (Apple.AllProduct) {
            ...
            setAllPhones(prevAllPhones => {
                if (!prevAllPhones.some(brand => brand[0]?.companyName === 'Apple')) {
                    return [...prevAllPhones, Apple.AllProduct];
                }
                return prevAllPhones;
            });
        }
    }, [Apple.AllProduct]);

    useEffect(() => {
        if (Realme.AllProduct) {
            setAllPhones(prevAllPhones => {
                if (!prevAllPhones.some(brand => brand[0]?.companyName === 'Realme')) {
                    return [...prevAllPhones, Realme.AllProduct];
                }
                return prevAllPhones;
            });
        }
                        ...
                        <Grid item xs={12} sx={{ display: "flex", justifyContent: "space-between" }}>
                            <Item square elevation={0}>
                                <Typography variant='h6'>
                                   ...
                                   {`${ele[0].companyName} SmartPhones`}
                                </Typography>
                            </Item>
                            <Item square elevation={0}>
                                <Link to={`MobileDetails/${ele[0].companyName}`}>
